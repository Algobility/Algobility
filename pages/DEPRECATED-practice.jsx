...
export default function practice() {
  ...
  const handleSearch = async () => {
    const response = await fetch('/api/search', {
      method: 'POST',
      body: JSON.stringify({ rank, topic }),
      headers: {
        ...
        <div className='w-full flex flex-col justify-start items-center rounded-lg '>
          ...
          <div className=' mt-12 w-3/4'>
            {cards.map((card, index) => (
              <div className='bg-neutral-600 h-32 rounded mb-8 w-full' key={index}>
                {card.title}
              </div>
            ))}
          </div>
...
export async function getServerSideProps() {
  // Fetch initial data from the server-side
  const rank = ''; // Set the default rank
  const topic = ''; // Set the default topic
  const searchResults = await getSortedPostsData(rank, topic);
