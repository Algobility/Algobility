...
Arrays help solve this by offering us a way to store a fixed-size list of variables. The variables within these lists are known as elements. Array elements must all be of the same type. They are identified by their position in the array rather than by variable names.
...
These examples use the int data type, but other data types can be used. Let’s try an example with strings:

```
string names[4] = {"Alice", "Bob", "Charlie", "David"}; // Array of 4 strings
